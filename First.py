        ...
        for j in range(len(Data[i])):
            ...
            if j == 1:
                ...
                for day in Repeat:
                    ...
                    Table[day].append(
                        [Data[i][0], Data[i][2], Data[i][3], Data[i][4]])
...
def Motivational_Quotes(filename):
    ...
    txt = myfile.read()
    lst = txt.split("\n")
    # Randomly picks and 1 quite from the motivation.txt file and returns it
    for x in range(10):
...
def format_for_printing_Timetable(Sorted_Table):
    ...
    big_day1 = max(len(Sorted_Table["MONDAY"]), len(Sorted_Table["TUESDAY"]), len(
        Sorted_Table["WEDNESDAY"]), len(Sorted_Table["THURSDAY"]))
    ...
    for a in range(big_day1):
        ...
        for i in Days_of_week1:
            ...
            if a > len(Sorted_Table[i])-1:
                ...
                statement = (
                    Sorted_Table[i][a][0]+" : "+Sorted_Table[i][a][1]+" -- "+Sorted_Table[i][a][2])
                row.append(statement)
        lst.append(row)

    big_day2 = max(len(Sorted_Table["FRIDAY"]), len(
        Sorted_Table["SATURDAY"]), len(Sorted_Table["SUNDAY"]))
    lst.append(divider)
        ...
        def __init__(self, root):
            ...
            for i in range(total_rows):
                for j in range(total_columns):
                    ...
                    self.e = Entry(root, width=27, fg="Royalblue4",
                                   font=('Arial', 12), bg="old lace")
        ...
        for a in range(len(Table[Day])-1):
            ...
            if a >= 0:
                ...
                if time(int(end_break[0]), int(end_break[1])) > time(int(next_st_break[0]), int(next_st_break[1])):
                    Clash = "True"
                    Statement = ("There is a clash  between" + "  ===> " + (" : ".join(
                        Table[Day][a][:-1])) + "  and   " + (" : ".join(Table[Day][a+1][:-1])) + "  on  " + Day)
                    FreeTimeSlots, Time_periods = Free_Time_Slots(
                        Table, Day_Start, Day_end)
                    Options = (
                        "You may choose some other time slot for your task from the following options", FreeTimeSlots)
                    print(Statement)

            # checking the first event of every day (flags it if lies before starting of Work Day)
            if a == 0:
                if int(st_break[0]) < int(Day_Start[:1]):
                    print(" : ".join(Table[Day][a][:-1]) +
                          " Event is Starting before Work Day")
            ...
            if a == (len(Table[Day])-2):
                if int(next_end_break[0]) >= int(Day_end[:2]) and int(next_end_break[1]) > int(Day_end[3:]):
                    print(" : ".join(Table[Day][a+1][:-1]) +
                          " Event is ending after Work Day")
    #
    if Clash == "True":
        print()
        print(Options)
...
def Productivity_Bar_Grpahs(Time_journal, objects):
    ...
    lst = ['red', 'blue', 'purple', 'green', 'lavender', "yellow", "orange", "blue", "fuchsia", "purple", 'purple', 'green', 'lavender',
           "yellow", 'blue', 'purple', 'green', 'lavender', "yellow", "orange", "blue", "purple", 'purple', 'green', 'lavender', "yellow"]
    plt.bar(y_pos, Time_journal, align='center',
            alpha=0.99, color=lst[:len(objects)+1])
    plt.xticks(y_pos, objects, rotation=10, fontweight='semibold',
...
def format_for_printing_Freetimeslots(FreeTimeSlots):
    ...
    big_day1 = max(len(FreeTimeSlots["MONDAY"]), len(FreeTimeSlots["TUESDAY"]), len(
        FreeTimeSlots["WEDNESDAY"]), len(FreeTimeSlots["THURSDAY"]))
    ...
    big_day2 = max(len(FreeTimeSlots["FRIDAY"]), len(
        FreeTimeSlots["SATURDAY"]), len(FreeTimeSlots["SUNDAY"]))
    lst.append(divider)
    lst.append(Days_of_week2)
...
def Free_Time_Slots(Sorted_Table, d_start, d_end):
    FreeTimeSlots, Time_periods = helper_free_time_slots(
        Sorted_Table, d_start, d_end)
    return(FreeTimeSlots, Time_periods)
...
def format_for_printing_Todo_list(Todolist):
    ...
    big_day1 = max(len(Todolist["MONDAY"]), len(Todolist["TUESDAY"]), len(
        Todolist["WEDNESDAY"]), len(Todolist["THURSDAY"]))
    ...
    big_day2 = max(len(Todolist["FRIDAY"]), len(
        Todolist["SATURDAY"]), len(Todolist["SUNDAY"]))
    lst.append(divider)
    lst.append(Days_of_week2)
    ...
    for i in range(len(Reminders)):
        for j in range(len(Reminders[i])):
            if j == 1:  # stopping at the second column where user has entered the days
                # calling function to convert (MT)-->[MONDAY,TUESDAY]
                Repeat = convert_days_to_fullform(Reminders[i][j])
                # Repeat is a list containing full form of days entered by user(repition days)
                for day in Repeat:
                    day = day.upper()
                    # coverting to uppercase
                    # appending event,start time,end time
                    Todolist[day].append([Reminders[i][0], Reminders[i][2]])
...
def Rewrite_Excel_File(Data, Heading, Reminders):
    ...
    for i in Reminders:
        Data.append(i)
    with open("testing.csv", mode="w", newline="") as csvfile:
...
def part2(Sorted_Table, Day, FreeTimeSlots, Time_periods, Data, Heading, Reminders, d_start, d_end, box):
    ...
    for key, value in Options.items():
        for i in value:
            if i[0] == num:
                Sorted_Table[key].append(
                    [work[0], i[1][:4], i[1][5:], work[3]])
                ...
                for a in range(len(Data)):
                    if Data[a][0] == work[0] and work[1] == Data[a][2]:
                        ...
                        if len(current_days_repeat) > 1:
                            new_days_repeat = current_days_repeat.replace(
                                Symbols[day_number], "")
                        ...
                        Data.append(
                            [work[0], Symbols[Days_of_the_week.index(key)], i[1][:5], n_end_time, work[3]])
...
print('-------Basic Table---------')
Table = Organize(Data)
# print('-------Time left till end of time_until_end_of_day---------')
...
print(Todolist)
print("-----Automatic_rescheduler-----")

Day = datetime.today().strftime('%A').upper()
print(Day)

# Automatic_rescheduler(Sorted_Table,Task_Numbers,Day,FreeTimeSlots,Time_periods,Data,Heading,Reminders,Day_Start,Day_end)
...
if len(Moti_Quote) > 170:
    ...
    label_frame = tk.Label(root, text=first_half, font=(
        "Arial semibold", 8)).place(x=70, y=250)
    label_frame = tk.Label(root, text=sec_half, font=(
        "Arial semibold", 8)).place(x=70, y=270)
    label_frame = tk.Label(root, text=third_half, font=(
        "Arial semibold", 8)).place(x=70, y=290)
    ...
    label_frame = tk.Label(root, text=Moti_Quote, font=(
        "Arial semibold", 8)).place(x=70, y=250)

 # This will create a LabelFrame for time and Date
label_frame = tk.Label(root, text=Time, font=(
    "Arial", 35), fg="brown4").place(x=222, y=10)
label_frame = tk.Label(root, text=Date, font=(
    "Arial", 12), fg="brown4").place(x=235, y=65)
label_frame = tk.Label(root, text=Day, font=(
    "Arial", 10), fg="brown4").place(x=255, y=85)
# label_frame = tk.Label(root, text="Have A Good",font=("Arial",8),fg="brown4",bg="bisque").place(x=370,y=473)
# label_frame = tk.Label(root, text="Day!",font=("Arial",8),fg="brown4",bg="bisque").place(x=395,y=495)
...
btn1 = tk.Button(root, text="Timetable", font=('Arial semibold', 12), image=timetable_img, compound=LEFT,
                 fg="green", relief=RAISED, command=lambda: (format_for_printing_Timetable(Sorted_Table)))
btn1.place(x=80, y=215)

photo = PhotoImage(file=r"freetimeslots.png")
free_img = photo.subsample(9, 9)

btn1 = tk.Button(root, text="Free-Time-Slots", font=('Arial semibold', 12), image=free_img, compound=LEFT,
                 fg="green", relief=RAISED, command=lambda:  format_for_printing_Freetimeslots(FreeTimeSlots))
btn1.place(x=210, y=215)

photo = PhotoImage(file=r"reminders.png")
reminders_img = photo.subsample(9, 10)

btn1 = tk.Button(root, text="Reminders", font=('Arial semibold', 12), fg="green", image=reminders_img,
                 compound=LEFT, relief=RAISED, command=lambda: format_for_printing_Todo_list(Todolist))
btn1.place(x=390, y=215)
...
label_frame = tk.Label(root, text="Task No.", font=(
    "Arial", 10), fg="brown4").place(x=75, y=355)

tasknumber = Entry(root)
tasknumber.place(x=80, y=380, height=15, width=45)
...
label_frame = tk.Label(root, text="Option No.", font=(
    "Arial", 10), fg="brown4").place(x=72, y=430)
optionnumber = Entry(root)
optionnumber.place(x=80, y=455, height=15, width=45)

b1 = Button(root, text="Clear", command=delete)
b1.place(x=80, y=400)

btn1 = tk.Button(root, text="Show Options", font=('Arial semibold', 8), fg="brown4", relief=RAISED, command=lambda: Automatic_rescheduler(
    Sorted_Table, Day, FreeTimeSlots, Time_periods, Data, Heading, Reminders, Day_Start, Day_end, box))
btn1.place(x=180, y=345)

box = Listbox(root, width=36, height=7)
box.place(x=135, y=380)

btn1 = tk.Button(root, text="Submit", font=('Arial semibold', 8), fg="brown4", relief=RAISED, command=lambda: part2(
    Sorted_Table, Day, FreeTimeSlots, Time_periods, Data, Heading, Reminders, Day_Start, Day_end, box))
btn1.place(x=260, y=345)
